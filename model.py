...
def dim_driver_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("driver_id","driver_license","driver_name","driver_rating","driver_phone")
...
def dim_vehicle_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("vehicle_id","vehicle_color","vehicle_make_id","vehicle_model","vehicle_type_id")
...
def dim_booking_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("ride_id","confirmation_number","dropoff_location_id","ride_status_id","dropoff_city_id","cancellation_reason_id","dropoff_address","dropoff_latitude","dropoff_longitude","dropoff_timestamp","total_fare","rating","booking_timestamp","pickup_address","pickup_latitude","pickup_location_id","pickup_longitude")
...
def fact_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("ride_id","payment_method_id","pickup_city_id","driver_id","passenger_id","vehicle_id","distance_miles","duration_minutes","base_fare","distance_fare","time_fare","surge_multiplier","subtotal","total_fare","tip_amount","rating","per_minute","per_mile")
    df = df.dropDuplicates(subset = ['ride_id'])
    return df
